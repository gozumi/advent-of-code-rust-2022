...
pub fn process_input(lines_buffer: Lines<BufReader<File>>) {
    ...
    for line in lines_buffer {
        ...
        if compressed_slice == "123456789" {
            ...
            println!("=========================");
            println!("slice -> {}", slice);
            stacks = perform_move(the_move, stacks);
            print_stacks(&stacks);
        }
    }
...
fn get_move(slice: &String) -> Move {
    ...
    return Move {
        n: stack_move_iterator.next().unwrap().parse::<usize>().unwrap(),
...
fn perform_move(the_move: Move, mut stacks: [Vec<char>; 10]) -> [Vec<char>; 10] {
    println!("-----------");
    println!("({}, {}, {})", the_move.n, the_move.from, the_move.to);
    println!("-----------");
    
    let from_stack = stacks[the_move.from].clone();
    let mut to_stack = stacks[the_move.to].clone();
    let start_of_move = from_stack.len() - the_move.n;
 
    let mut new_from_stack: Vec<char> = vec![];
    
    for i in 0..start_of_move {
        let crate_to_be_moved = from_stack[i];
        new_from_stack.push(crate_to_be_moved);
    }
    
    for i in start_of_move..from_stack.len()  {
        let crate_to_be_moved = from_stack[i];
        to_stack.push(crate_to_be_moved);        
    }

    stacks[the_move.from] = new_from_stack;
    stacks[the_move.to] = to_stack;
